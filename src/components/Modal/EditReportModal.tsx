...
const EditReportModal = (props: EditReportProps) => {
	...
	const send = (data: editReportFormData) => {
		const request = {
			...data,
			attachments: attachedFile as string,
		};
		mutate(request);
	};
			...
			<Dialog.Portal>
				...
				<Dialog.Content className="w-[608px] rounded-lg border border-gray-200 fixed pt-4 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white overflow-hidden">
					<div className="w-full px-6 pb-4 border-b-[1px] border-gray-200 flex items-center flex-row justify-between">
						<Dialog.Title className="font-semibold text-2xl">Editar relatório</Dialog.Title>
					...
					<div className="w-full px-6 py-6 flex flex-col gap-4">
						{isLoadingPatientReportData ? (
							<Load
								divProps={{
									className: 'w-full h-[488px] flex items-center justify-center relative bg-gray-500-50',
								}}
							/>
						) : savingChanges ? (
							<Load
							...
							<>
								<div className="w-full flex justify-between">
									<div className="flex items-center gap-2">
										...
										<p className="text-base font-normal text-brand-standard-black">{data?.id}</p>
									</div>
									<div className="flex flex-col">
										<div className="flex items-center gap-1">
											<span className="text-sm font-semibold text-brand-standard-black">Data de criação:</span>
											<p className="text-base font-normal text-brand-standard-black">{data?.createdAt}</p>
										</div>
										<div className="flex items-center gap-1">
											<span className="text-sm font-semibold text-brand-standard-black">Data da última edição:</span>
											<p className="text-base font-normal text-brand-standard-black">{data?.updatedAt}</p>
										</div>
									</div>
								</div>
								<form onSubmit={handleSubmit(send)} className="w-full flex flex-col h-360 gap-3">
									<div className="w-full flex flex-col gap-6">
										<div className="w-full flex flex-row gap-3">
											<div className="w-[184px] flex flex-col gap-3">
												...
												<Select
													...
													isSearchable={false}
													options={turnOptions}
													value={selectShiftValue ? turnOptions.find((x) => x.value === selectShiftValue) : selectShiftValue}
													onChange={(option) => selectShiftOnChange(option ? option.value : option)}
													{...restSelectShift}
												/>
											</div>
										...
										<div className="w-full flex flex-col gap-3">
											...
											<div>
												...
												></textarea>
												{errors.report_text && (
													<span className="w-full mt-1 flex items-center font-normal text-sm text-red-500">{errors.report_text.message}</span>
												)}
											</div>
										</div>
									</div>
									<div className="w-full flex flex-col gap-3">
										...
										<div className="w-full flex justify-between">
											<div className="w-full flex">
												...
												<input type="file" accept=".doc, .docx, .pdf" id="patient-photo-file" className="hidden" onChange={handleFile} />
											</div>
