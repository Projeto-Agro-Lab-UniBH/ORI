...
type DeleteVaccineModalProps = {
  id: string;
}

const DeleteVaccineModal: React.FC<DeleteVaccineModalProps> = ({ id: vaccineId }) => {
  const [open, setOpen] = useState<boolean>(false);
  const [isLoading, setIsLoading] = useState<boolean>(false);

  const { mutate } = useMutation({
    mutationKey: ["delete-vaccine-by-id"],
    mutationFn: async () => {
      try {
        setIsLoading(true)
  ...
  const acceptedDeleteVaccine = (event: FormEvent<HTMLButtonElement>) => {
